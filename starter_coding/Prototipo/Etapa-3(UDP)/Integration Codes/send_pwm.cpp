...
#define ELIPSE_X 2 * 15 * 15
#define ELIPSE_Y 15 * 15
...
char *server = "192.168.0.18"; /* change this to use a different server */
char buf[BUFLEN];

void watch() {
  /* now let's send the messages */

  cout << "Sending packet" << i << "to " << server << " port " << SERVICE_PORT
       << endl;
  strcpy(buf, to_send);
...
void setNext(int x, int y, Mat &img) {
  circle(img, Point(goalX[estado], goalY[estado]), 2, Scalar(0, 0, 255), 8, 8,
         0);

  arrowedLine(img, Point(x, y), Point(goalX[estado], goalY[estado]),
              Scalar(0, 0, 255), 5, 8, 0, 0.5);

  if ((x - goalX[estado]) * (x - goalX[estado]) / (ELIPSE_X) +
              (y - goalY[estado]) * (y - goalY[estado]) / (ELIPSE_Y) <=
          1 ||
      (x - goalX[estado]) * (x - goalX[estado]) +
              (y - goalY[estado]) * (y - goalY[estado]) >
          (x - goalX[estado + 1]) * (x - goalX[estado + 1]) +
              (y - goalY[estado + 1]) * (y - goalY[estado + 1])) {
    cout << estado << endl;
    circle(img, Point(goalX[estado], goalY[estado]), 2, Scalar(255, 0, 0), 8, 8,
           0);
    estado++;

    if (goalX.size() == estado) {
      estado = 0;
    }
  }

  // if (((abs(goalX[estado] - x) < CHANGE_POINT_STATUS_X && abs(goalY[estado] -
  // y) < CHANGE_POINT_STATUS_Y) && goalX.size() > estado+1) ||
  // 			((abs(goalX[estado] - x)) >= (abs(goalX[estado+1] -
  // x)))) { 	cout << estado << endl; 	circle(img, Point(goalX[estado],
  // goalY[estado]), 2, Scalar(255, 0, 0), 8, 8, 0); 	estado++; } else if
  // (goalX.size() == estado+1){ 	estado = 0;
  // }
  ...
  cout << "PWM0 = " << pwm[0] << " PWM1 = " << pwm[1] << " PWM2 = " << pwm[2]
       << " PWM3 = " << pwm[3] << endl;

  if (pwm[3] > 10) pwm[3] += 50;

  if (pwm[2] > 10) pwm[2] += 50;

  pwm[0] = map1(pwm[0], 0, 240, 0, 1);   // cima
  pwm[1] = map1(pwm[1], 0, 240, 0, -1);  // baixo
  pwm[2] = map1(pwm[2], 0, 300, 0, -1);  // esquerda
  pwm[3] = map1(pwm[3], 0, 300, 0, 1);   // direita

  new_send_y = pwm[0] + pwm[1];
  new_send_x = pwm[2] + pwm[3];
  ...
  sprintf(temp0, "%d", new_send_x);
  sprintf(to_send, "%s", temp0);
  strcat(to_send, "-");
  sprintf(temp1, "%d", new_send_y);
  strcat(to_send, temp1);
...
void getMaestroPontos() {
  // ifstream myfile(
  //     "/home/barbosa/Documentos/Pesquisa/Maestro/Data/Compasso2x4-2d.csv");
  ifstream
  myfile("/home/luiz/Documents/Projects/maestro/Data/Compasso2x4-2d.csv");
  ...
  if (myfile.is_open()) {
    while (myfile.good()) {
      ...
      maestroX.push_back(atof(temp1));
      maestroY.push_back(atof(temp2));
    }
...
void showTrajetory(Mat &frame) {
  goalX.clear();
  goalY.clear();
  /*for(vector<float>::iterator it = maestroX.begin(); it != maestroX.end();
  ++it) { goalX.push_back( (int)(x_ref + x_ref* (*it) ) );
  }
<<<<<<< HEAD
=======

>>>>>>> 04fda83def5b2e31ba0467228297d3fe4a0a1eb5
  ...
  for (vector<float>::size_type i = 0; i < maestroX.size(); i++) {
    ...
    goalY.push_back(
        (int)(FRAME_HEIGHT - (maestroY[i] * (FRAME_HEIGHT - y_ref))));

    // rectangle(frame, Point(goalX[i]-CHANGE_POINT_STATUS_X,
    // goalY[i]-CHANGE_POINT_STATUS_Y),
    // 					Point(goalX[i]+CHANGE_POINT_STATUS_X,
    // goalY[i]+CHANGE_POINT_STATUS_Y), Scalar(0, 255, 0), 1, 8, 0);

    ellipse(frame, Point(goalX[i], goalY[i]),
            Size(CHANGE_POINT_STATUS_X, CHANGE_POINT_STATUS_Y), 0, 0, 360,
            Scalar(0, 255, 0), 1, 8);
    ...
    if (estado == i) {
      ...
      circle(frame, Point(goalX[i], goalY[i]), 2, Scalar(255, 255, 255), 8, 8,
             0);
    }
  }
}
  ...
  if (hierarchy.size() > 0) {
    ...
    if (numObjects < MAX_NUM_OBJECTS) {
      for (int index = 0; index >= 0; index = hierarchy[index][0]) {
        ...
        // bad filter  we only want the object with the largest area so we safe
        // a reference area each  iteration and compare it to the area in the
        // next iteration.
      ...
      putText(frame, "TOO MUCH NOISE! ADJUST FILTER", Point(0, 50), 1, 2,
              Scalar(0, 0, 255), 2);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  // VideoCapture
  // capture("/home/barbosa/Documentos/Pesquisa/Maestro/Videos/IMG_3260.mp4");
